...
// CreateDevtmpfsFiles creates device special files in a representing all
...
func CreateDevtmpfsFiles(ctx context.Context, dev *devtmpfs.Accessor) error {
	for minor, name := range map[uint32]string{
		...
		if err := dev.CreateFile(ctx, name, vfs.CharDevice, linux.MEM_MAJOR, minor, 0666 /* mode */); err != nil {
			return err
		}
	}
	return nil
