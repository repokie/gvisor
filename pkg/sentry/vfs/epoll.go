...
const (
	// _EP_PRIVATE_BITS is fs/eventpoll.c:EP_PRIVATE_BITS, the set of all bits
	// in an epoll event mask that correspond to epoll flags rather than I/O
	// events.
	_EP_PRIVATE_BITS = linux.EPOLLONESHOT | linux.EPOLLET // EPOLLWAKEUP and EPOLLEXCLUSIVE are unsupported
)
...
func (ep *EpollInstance) mightPollRecursive(ep2 *EpollInstance, remainingRecursion int) bool {
	...
	for key := range ep.interest {
		nextep, _ := key.file.impl.(*EpollInstance)
		if nextep == nil {
			continue
		}
		if nextep == ep2 {
