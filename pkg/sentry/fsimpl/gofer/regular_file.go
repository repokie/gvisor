...
type regularFileFD struct {
	...
	flags uint32 // status flags; accessed atomically
...
func (fd *regularFileFD) PWrite(ctx context.Context, src usermem.IOSequence, offset int64, opts vfs.WriteOptions) (int64, error) {
	...
	if d.fs.opts.interop != InteropModeShared {
		...
		d.touchCMtime(ctx, fd.vfsfd.Mount())
	}
	rw := getDentryReadWriter(ctx, d, offset)
...
func (rw *dentryReadWriter) ReadToBlocks(dsts safemem.BlockSeq) (uint64, error) {
	...
	// unreliable), read directly from dentry.handle without locking
	// dentry.dataMu.
	rw.d.handleMu.RLock()
	if (rw.d.handle.fd >= 0 && !rw.d.fs.opts.forcePageCache) || rw.d.fs.opts.interop == InteropModeShared {
...
func (rw *dentryReadWriter) WriteFromBlocks(srcs safemem.BlockSeq) (uint64, error) {
	...
	// If we have a mmappable host FD (which must be used here to ensure
	// coherence with memory-mapped I/O), or if InteropModeShared is in effect
	// (which prevents us from caching file contents), write directly to
...
func (fd *regularFileFD) ConfigureMMap(ctx context.Context, opts *memmap.MMapOpts) error {
	...
	// FIXME(jamieliu): make vfs.FileDescription implement memmap.MappingIdentity
	return syserror.ENODEV
}
